function outPath = GridDigitViz(symVal, baseRadix, outFile, nVal, varargin)
...
if nC == 1
    ...
    opts.Colormap = repmat(opts.Colormap, baseRadix, 1);
else
    old = linspace(0,1,nC);
    new = linspace(0,1,baseRadix);
    opts.Colormap = interp1(old, opts.Colormap, new);
end

% --- Integer digit extraction ---
baseSym = sym(baseRadix);
...
if intPart == 0
    ...
    while tmp > 0 && it < loopCap
        [q,r] = quorem(tmp, baseSym);
        intDigits = [double(r) intDigits]; %#ok<AGROW>
        tmp = q; it = it + 1;
    end
    if it >= loopCap
...
imgD(validMask) = max(0, min(baseRadix-1, floor(imgD(validMask))));
...
colormap(ax, opts.Colormap(1:baseRadix,:));
caxis(ax,[0 baseRadix-1]);
...
cb.Label.String = sprintf('Digits (base %d)', baseRadix);

if isempty(opts.Title)
    if opts.FractionDigits > 0
        ttl = sprintf('Schizo number sqrt(f(%d)) base %d (frac %d)', nVal, baseRadix, opts.FractionDigits);
    else
        ttl = sprintf('Schizo number sqrt(f(%d)) base %d', nVal, baseRadix);
    end
else
    ttl = opts.Title;
end
